...
import {
  ...
  createSection, getBlockDefinition,
} from "@/lib/blocks";
...
export default function PageEditor({
  page, locale = "ar", isTranslation = false, hasExistingTranslation = false,
}: {
  page: PageData; locale?: string; isTranslation?: boolean; hasExistingTranslation?: boolean;
}) {
  ...
  const undo = () => { if (historyIdx > 0) { setSections(history[historyIdx - 1]); setHistoryIdx(i => i - 1); setIsDirty(true); } };
  const redo = () => { if (historyIdx < history.length - 1) { setSections(history[historyIdx + 1]); setHistoryIdx(i => i + 1); setIsDirty(true); } };
  ...
  function duplicateSection(id: string) {
    const idx = sections.findIndex(s => s.id === id);
    const clone = { ...structuredClone(sections[idx]), id: crypto.randomUUID() };
  ...
  const filteredBlocks = BLOCK_DEFINITIONS.filter(def => {
    if (blockSearch) return def.label.toLowerCase().includes(blockSearch.toLowerCase()) || def.description.toLowerCase().includes(blockSearch.toLowerCase());
    ...
    >
      <span>✨ ترجمة تلقائية (Google)</span>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button onClick={redo} disabled={historyIdx >= history.length - 1} title="Redo (⌘⇧Z)"
            className="p-2 rounded-lg text-[#9CA3AF] hover:text-[#374151] hover:bg-[#F3F4F6] disabled:opacity-30 transition">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="15 1 21 7 15 13"/><path d="M9 18H3v-5"/><path d="M21 7H8a5 5 0 0 0-5 5"/></svg>
          </button>
          ...
          {(["desktop","tablet","mobile"] as const).map(v => (
            <button key={v} onClick={() => setViewport(v)} title={v}
              ...
              <Icon name={v === "desktop" ? "monitor" : v === "tablet" ? "tablet" : "smartphone"} size={16} />
            </button>
          ))}
          <div className="w-px h-5 bg-[#E5E7EB] mx-1" />
          <a href={page.slug === "home"
              ? (locale === "ar" ? "/" : `/${locale}/`)
              : (locale === "ar" ? `/${page.slug}` : `/${locale}/${page.slug}`)}
            target="_blank"
            ...
            {([["layers","Layers","layers"] as const, ["blocks","+ Add Block","layout-grid"] as const]).map(([val, label, icon]) => (
              <button key={val} onClick={() => setLeftTab(val)}
                className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-[11px] font-bold transition border-b-2 ${
                  leftTab === val ? "text-[#6366F1] border-[#6366F1]" : "text-[#9CA3AF] border-transparent hover:text-[#374151]"
                }`}>
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
              <div className="p-3 pb-2 border-b border-[#F3F4F6]">
                <div className="relative">
                  ...
                  <input value={blockSearch} onChange={e => { setBlockSearch(e.target.value); setBlockCat("all"); }}
              ...
              {!blockSearch && (
                <div className="px-3 py-2 flex flex-wrap gap-1 border-b border-[#F3F4F6]">
                  {BLOCK_CATEGORIES.map(cat => (
                    <button key={cat.id} onClick={() => setBlockCat(cat.id)}
                  ...
                  {filteredBlocks.map(def => (
                    <button key={def.type} onClick={() => addSection(def.type)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[#9CA3AF] text-[10px] mt-0.5 leading-tight">{def.description}</div>
                      </div>
                    </button>
                  ))}
                  {filteredBlocks.length === 0 && (
                    <div className="text-center py-8 text-[#9CA3AF] text-xs">No blocks found</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="p-3">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[11px] font-bold text-[#9CA3AF] uppercase tracking-wider">{sections.length} Blocks</span>
                ...
                {sections.length === 0 ? (
                  <div className="text-center py-10">
                    <div className="w-12 h-12 rounded-2xl bg-[#F3F4F6] flex items-center justify-center mx-auto mb-3">
                      <Icon name="layout-grid" size={22} className="text-[#D1D5DB]" />
                    </div>
                    <p className="text-[#9CA3AF] text-xs">No blocks yet</p>
                    <button onClick={() => setLeftTab("blocks")} className="text-[#6366F1] text-xs font-semibold mt-1 hover:underline">
                    ...
                    {sections.map((s, idx) => {
                      ...
                      return (
                        <div key={s.id}
                          ...
                          <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${isActive ? "bg-[#6366F1] text-white" : "bg-[#F3F4F6] text-[#6B7280]"}`}>
                          ...
                          <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition">
                            ...
                            <button onClick={e => { e.stopPropagation(); moveSection(s.id, "down"); }} disabled={idx === sections.length - 1}
                              className="p-0.5 rounded hover:bg-[#E0E7FF] text-[#6366F1] disabled:opacity-20">
        ...
        <div ref={canvasRef} className="flex-1 overflow-y-auto flex flex-col items-center py-6 px-4">
          {sections.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center max-w-xs">
              ...
              <h3 className="text-[#374151] font-bold text-lg mb-2">Start Building</h3>
              <p className="text-[#9CA3AF] text-sm mb-4">Click "+ Add Block" to add your first section</p>
              <button onClick={() => setLeftTab("blocks")}
            ...
            <div className="h-[44px] flex items-center justify-between px-4 border-b border-[#E2E5ED] shrink-0">
              {selected && selectedDef ? (
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-xs font-bold text-[#374151] truncate">{selectedDef.label}</span>
                </div>
              ) : (
                <span className="text-xs font-bold text-[#9CA3AF]">Properties</span>
              )}
              <button onClick={() => setRightCollapsed(true)} className="text-[#D1D5DB] hover:text-[#6B7280] transition p-1 rounded">
                <Icon name="x" size={14} />
              </button>
            </div>
            ...
            {selected ? (
              <div className="flex-1 overflow-y-auto">
                <div className="flex items-center gap-1 px-3 py-2 border-b border-[#F3F4F6]">
                  ...
                  <button onClick={() => moveSection(selected.id, "down")} disabled={sections.indexOf(selected) === sections.length - 1}
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg hover:bg-[#F3F4F6] disabled:opacity-30 text-[#6B7280] text-[11px] font-medium transition">
                    <Icon name="arrow-down" size={12} />Down
                  </button>
                  <button onClick={() => duplicateSection(selected.id)}
                  ...
                  <button onClick={() => deleteSection(selected.id)}
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center px-6">
                ...
                <p className="text-[#374151] font-semibold text-sm mb-1">No block selected</p>
                <p className="text-[#9CA3AF] text-xs">Click any block on the canvas to edit its properties</p>
              </div>
            )}
          </div>
        )}
    ...
    <div
      ...
      {isSelected && (
        <div className="absolute top-0 right-0 z-10 flex items-center gap-0.5 p-1.5 bg-[#6366F1] shadow-sm" style={{ borderBottomLeftRadius: 10 }}>
          {[
            { icon: "arrow-up", label: "Move Up", action: onMoveUp, disabled: idx === 0 },
            { icon: "arrow-down", label: "Move Down", action: onMoveDown, disabled: idx === total - 1 },
            { icon: "copy", label: "Duplicate", action: onDuplicate, disabled: false },
