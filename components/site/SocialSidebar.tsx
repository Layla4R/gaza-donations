...
const ITEMS = [
  ...
  },
  {
    ...
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.848L1.258 2.25H8.08l4.259 5.63 5.905-5.63zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>,
    getHref: (v: string) => v.startsWith("http") ? v : `https://x.com/${v}`,
  },
  {
    ...
    getHref: (v: string) => v.startsWith("http") ? v : `https://instagram.com/${v}`,
  },
  {
    ...
    getHref: (v: string) => v.startsWith("http") ? v : `https://tiktok.com/@${v}`,
  },
  {
...
export default function SocialSidebar({ whatsapp, facebook, twitter, instagram, tiktok, youtube, linkedin, position = "right" }: Props) {
  ...
  return (
    <div className={`fixed top-1/2 -translate-y-1/2 z-50 flex flex-col items-${isRight ? "end" : "start"} ${isRight ? "right-0" : "left-0"}`}>
      {visible.map((item, idx) => {
        const val = values[item.key]!;
        return (
          <a
            key={item.key}
            href={item.getHref(val)}
            target="_blank"
            rel="noopener noreferrer"
            title={item.label}
            style={{ backgroundColor: item.color, transitionDelay: `${idx * 35}ms` }}
            className={`group flex items-center overflow-hidden text-white shadow-lg transition-all duration-300 mb-px
              ${isRight
                ? expanded ? "flex-row-reverse w-36 px-3 rounded-l-xl" : "w-11 rounded-l-xl"
                : expanded ? "w-36 px-3 rounded-r-xl" : "w-11 rounded-r-xl"
              } h-11`}
          >
            <span className={`flex items-center justify-center transition-all duration-300 ${expanded ? "w-5 shrink-0" : "w-full"}`}>{item.icon}</span>
            <span className={`text-xs font-bold whitespace-nowrap transition-all duration-300 ${isRight ? "ml-2" : "mr-2"} ${expanded ? "opacity-100 max-w-xs" : "opacity-0 max-w-0 overflow-hidden"}`}>
              {item.label}
            </span>
          </a>
        );
      ...
      <button
        ...
        className={`mt-1 w-11 h-8 bg-ink/60 hover:bg-ink text-white ${isRight ? "rounded-l-lg" : "rounded-r-lg"} flex items-center justify-center text-xs transition`}
