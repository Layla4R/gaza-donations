...
const DEFAULT_SLIDES: Slide[] = [
  {
    ...
    title_ar: "مؤسسة 4Relief الإنسانية العالمية للإغاثة الطارئة",
    title_en: "4Relief | International Humanitarian Foundation & Emergency Relief",
    title_fr: "4Relief | Fondation Humanitaire Internationale & Secours d'Urgence",
    title_tr: "4Relief | Uluslararası İnsani Yardım Vakfı",

    subtitle_ar:
      "منصة تبرعات شفافة وآمنة لدعم الأسر المحتاجة والنازحين حول العالم.",
    subtitle_en:
      "A transparent, secure humanitarian foundation supporting families in need worldwide.",
    subtitle_fr:
      "Une plateforme de dons transparente pour soutenir les familles dans le besoin.",
    subtitle_tr:
      "Dünya genelinde ihtiyaç sahibi aileleri destekleyen şeffaf insani yardım platformu.",
  },

  {
    ...
    title_ar: "يدٌ تمتد لكل محتاج — 4Relief Humanitarian Foundation",
    title_en: "A Hand for Every Person — 4Relief Humanitarian Foundation",
    title_fr:
      "Une Main Tendue à Chaque Personne — Fondation 4Relief",
    title_tr:
      "İhtiyaç Sahibi Herkes İçin Bir El — 4Relief İnsani Yardım",

    subtitle_ar:
      "تبرعك يصل مباشرة للمستحقين دون وسيط بشفافية ماليّة 100%.",
    subtitle_en:
      "Your donation reaches beneficiaries directly with full transparency and 100% audit.",
    subtitle_fr:
...

  {
    ...
    title_ar: "كل دولار يغير حياة — 4Relief Humanitarian Foundation",
    title_en: "Every Dollar Changes a Life — 4Relief Humanitarian Foundation",
    title_fr: "Chaque Euro Change une Vie — 4Relief",
    title_tr: "Her Dolar Bir Hayat Değiştirir — 4Relief",
...
export default function HeroSection({
    ...
    adminSlides.length > 0
      ? adminSlides
          .map((slide: any) => ({
            ...
            title_ar:
              slide?.title_ar || slide?.title || slide?.headline || "",
            title_en:
              slide?.title_en || slide?.title || slide?.headline || "",
            title_fr:
              slide?.title_fr || slide?.title || slide?.headline || "",
            title_tr:
              slide?.title_tr || slide?.title || slide?.headline || "",

            subtitle_ar:
              slide?.subtitle_ar || slide?.subtitle || slide?.subheading || slide?.description || "",
            subtitle_en:
              slide?.subtitle_en || slide?.subtitle || slide?.subheading || slide?.description || "",
            subtitle_fr:
              slide?.subtitle_fr || slide?.subtitle || slide?.subheading || slide?.description || "",
            subtitle_tr:
              slide?.subtitle_tr || slide?.subtitle || slide?.subheading || slide?.description || "",
          }))
  ...
  useEffect(() => {
    if (slides.length <= 1 || hovered || showDetails) {
      return;
    }

    const timer = window.setInterval(next, 6000);
    return () => window.clearInterval(timer);
  }, [next, slides.length, hovered, showDetails]);
  ...
  async function pay(provider: "stripe" | "paypal") {
    if (!name.trim() || !email.trim()) {
      setPayError(
        t("donate.name", "الاسم والبريد مطلوبان", "Name and email required", "Nom et email requis", "Ad ve e-posta gerekli")
      );
      return;
    }
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) {
      setPayError(
        t("cart.invalid_email", "بريد إلكتروني غير صحيح", "Invalid email address", "Email invalide", "Geçersiz e-posta")
      );
      return;
    }
    ...
    try {
      ...
      setPayError(d?.error || t("common.error", "حدث خطأ", "An error occurred", "Une erreur s'est produite", "Bir hata oluştu"));
    } catch {
  ...
  function handleAddToCart() {
    ...
    try {
      ...
      const item = {
        ...
        amount: final,
        frequency: freq === "ONE_TIME" ? "one_time" : "monthly",
        campaignId: null,
      };
    ...
    >
      {slides.map((sl, index) => (
        <div
          ...
          style={{
            opacity: index === current && !animating ? 1 : 0,
            zIndex: 0,
          }}
        >
          <Image
            src={sl.image}
            alt={
              sl[`title_${locKey}` as keyof Slide] ||
              sl.title_ar ||
      ...
      <div className="relative z-10 flex flex-1 items-center py-12 lg:py-16">
        <div className="mx-auto w-full max-w-screen-xl px-6">
          <div className="max-w-2xl">
            ...
            {/* 🌟 H1 Title Optimized for Search Engine Relevance */}
            <h1
            ...
            <div className="flex flex-wrap items-center gap-6 border-t border-white/10 pt-2">
              {[
                ...
                <div key={index} className="flex items-center gap-2 text-xs font-medium text-white/75">
                  <Icon name={item.icon} size={15} className="text-white/90" />
                  {t(`hero.trust${index}`, item.ar, item.en, item.fr, item.tr)}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mx-auto max-w-screen-xl px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex shrink-0 rounded-xl border border-white/15 bg-white/15 p-1">
              {(["ONE_TIME", "MONTHLY"] as const).map((value) => (
                <button
                  ...
                  style={{ color: freq === value ? primary : undefined }}
                >
                  {value === "ONE_TIME"
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
              {QUICK_AMOUNTS.map((value) => (
                <button
                  ...
                  style={{ color: final === value && !custom ? primary : undefined }}
                >
                  ${value}
                </button>
              ))}
            ...
            <div className="relative shrink-0">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-white/80">$</span>
              <input
                type="number"
                min={1}
                value={custom}
            ...
            <button
              type="button"
              onClick={() => { setShowDetails((value) => !value); setPayError(""); }}
            ...
            >
              ...
              {freq === "MONTHLY" && <span className="text-[10px] opacity-75">/{t("donate.monthly", "شهر", "mo", "mois", "ay")}</span>}
            </button>
          </div>
                ...
                <div className="flex shrink-0 flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 rounded-xl border px-4 py-2 text-xs font-bold transition ${
                      cartAdded ? "border-emerald-400 bg-emerald-500/20 text-emerald-100" : "border-white/20 bg-white/15 text-white hover:bg-white/25"
                    }`}
                  >
                    <Icon name={cartAdded ? "check" : "layers"} size={14} />
                    {cartAdded ? t("added", "أُضيف ✓", "Added ✓", "Ajouté ✓", "Eklendi ✓") : t("add_to_cart", "السلة", "Cart", "Panier", "Sepet")}
                  </button>
                </div>
              </div>

              {payError && <p className="mt-2 text-xs font-medium text-red-200">{payError}</p>}
            </div>
          )}
        </div>
      </div>
