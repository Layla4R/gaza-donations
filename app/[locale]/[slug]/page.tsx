...
export default async function DynamicPage({ params }: { params: { slug: string; locale: string } }) {
  ...
  const [page, campaigns, dict] = await Promise.all([
  ...
  return (
    <div className="bg-white">
      {/* 🌟 تم تغيير الكلاس هنا من bg-brand-gradient إلى bg-brand ليصبح لون primary الصريح */}
      <header className="relative py-12 sm:py-20 bg-brand text-center overflow-hidden">
      ...
      <div className="bg-white">
        {hasCustomSections ? (
          sections.map((section) => (
            <BlockRenderer key={section.id} section={section} context={{ campaigns, whiteBackground: true, locale, dict }} />
          ))
        ) : isLegalPage ? (
